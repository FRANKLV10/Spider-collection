...
logging.basicConfig(filename='./log/'+str(day)+'err_log.txt', level=logging.WARNING,format='%(asctime)s - %(levelname)s - %(message)s')
...
def search_all(name):
    ...
    if first_list != None:
        print("开始获取二级子公司")
        for i in first_list:
            second_company_data = get_detail(i)
